...
- Theme: unconfigured `--style auto` is palette-adaptive (zero truecolor/painted backgrounds); `dark|light|notty` select fixed bases. Startup-only partial theme YAML overlays the selected base; `notty` ignores custom colors. Discover `theme.yaml` then `theme.yml` beside config; config `theme` paths are config-relative, CLI `--theme PATH` paths cwd-relative and win. Never create a theme file; validate only the selected file. See `docs/configuration.md` and `theme.example.yaml`.
...
- Theme role markers annotate only render-local AST segments/public string hooks; link-bearing emphasis subtrees use public inline elements before escaped render-local reinsertion. Stock strike containers keep their existing flattened link presentation. Compose fresh output before cache/metadata; public indentation bounds and stock trailing-reset boundaries protect layout padding (heading-owned padding retains heading overrides). Top-level cells strip markers before measurement. Stock nested tables retain zero-width markers through stock layout (invariance-tested), then strip during complete-container composition; malformed/unbalanced markers error. Chroma registry registration and all readmd stock render reads share one lock, avoiding shared `charm` styles. Explicit code backgrounds use public formatter wrappers and intrinsic rectangles outside margins; painted rectangles display each TAB as four spaces, never reflowing or changing raw source.
