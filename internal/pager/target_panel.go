...
func (m *Model) targetVisibleRows() int {
	if m.targets.list.panelHeight == 1 {
		return 1
	}
	return m.targets.list.panelHeight - 4 // borders and two focus-detail rows
}

func (m *Model) targetPanelY() int { return m.height - 1 - m.targets.list.panelHeight }

func (m *Model) focusedTarget() (hintTarget, bool) {
	candidates := m.targetCandidates()
	if m.targets.list.focus < 0 || m.targets.list.focus >= len(candidates) {
		return hintTarget{}, false
	}
	return candidates[m.targets.list.focus], true
}

func (m *Model) resetTargetFocus() {
	m.targets.vimium.focus = 0
	m.targets.list.focus, m.targets.list.top, m.targets.list.detailPage = 0, 0, 0
	m.applySearchView()
}
...
func (m *Model) moveTargetFocus(delta int) {
	...
	m.targets.list.focus = min(max(0, m.targets.list.focus+delta), count-1)
	m.targets.list.top = min(m.targets.list.top, m.targets.list.focus)
	m.targets.list.top = max(m.targets.list.top, m.targets.list.focus-m.targetVisibleRows()+1)
	m.targets.list.detailPage = 0
	m.applySearchView()
}
...
func (m *Model) targetPanelRows() []string {
	...
	page := min(m.targets.list.detailPage, m.targetDetailPageCount()-1)
	if m.targets.list.panelHeight == 1 {
	...
	for i := range m.targetVisibleRows() {
		index := m.targets.list.top + i
		line := ""
		if index < len(candidates) {
			target := candidates[index]
			key := target.label
			if index == m.targets.list.focus {
...
func (m *Model) targetStatus() string {
	...
	if _, ok := m.focusedTarget(); ok {
		selected = m.targets.list.focus + 1
	}
	...
	if m.targets.list.panelHeight == 1 && m.targetDetailPageCount() > 1 {
